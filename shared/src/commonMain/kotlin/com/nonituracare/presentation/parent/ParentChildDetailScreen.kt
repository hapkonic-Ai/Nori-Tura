...
fun ParentChildDetailScreen(
    patientId: String,
    viewModel: ParentChildDetailViewModel = viewModel(key = patientId) { ParentChildDetailViewModel(patientId) },
    onBack: () -> Unit,
    ...
    NorituraScaffold(
        ...
        when (val state = uiState) {
            is ParentChildDetailViewModel.UiState.Loading -> {
                LoadingState(modifier = Modifier.fillMaxSize())
            }

            is ParentChildDetailViewModel.UiState.Error -> {
                ErrorState(
            ...
            is ParentChildDetailViewModel.UiState.Success -> {
                ChildDetailContent(
...
private fun ChildDetailContent(
    profile: ParentChildDetailViewModel.ChildProfile,
    onNavigateToConsentView: (String) -> Unit,
    ...
    LazyColumn(
        ...
        item { ChildInfoCard(child = child) }
        ...
        if (profile.consentForms.isEmpty()) {
            item {
                InlineMessage("No consent forms yet.")
            }
        } else {
            item {
        ...
        item {
            ...
            if (uploadedImages.isNotEmpty() || uploadedPdfs.isNotEmpty()) {
                ...
                Button(
                    onClick = {
                        val childId = child.id ?: return@Button
            ...
            if (profile.documents.isEmpty()) {
                InlineMessage("No health records uploaded yet.")
            } else {
...
@Composable
private fun ChildInfoCard(child: PatientDto) {
        ...
        Card(
            ...
            Row(verticalAlignment = Alignment.CenterVertically) {
                ...
                Column {
                    ...
                    if (!child.hospitalName.isNullOrBlank()) {
                        Text(
                            text = child.hospitalName ?: "",
